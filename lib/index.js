...
function PgRestify(config, postInitFunction) {
  ...
  this.pgConnectionString = config.pgConnectionString;
  this.pg = pg;
      ...
      self.validateTable(table, function(err) {
        ...
        if (req.query.orderBy) {
          ...
          if (columns.indexOf(orderByColumn) === -1) {
            return next(new restify.BadRequestError('Invalid orderBy field "' + req.query.orderBy + '"'));
          } else {
        ...
        if (req.query.orderByDesc) {
          if (req.query.orderByDesc === 'true') {
            ...
            return next(new restify.BadRequestError('Invalid orderByDesc value "' + req.query.orderByDesc + '"'));
        ...
        if (typeof req.query.page !== 'undefined') {
          ...
          if (isNaN(req.query.page)) {
            return next(new restify.BadRequestError('Invalid page value "' + req.query.page + '"'));
          }

          page = parseInt(req.query.page);

          if (page < 1)  {
            return next(new restify.BadRequestError('page must be > 0 but was "' + page + '"'));
          }
        }
        ...
        if (typeof req.query.pageSize !== 'undefined') {
          ...
          if (isNaN(req.query.pageSize)) {
            return next(new restify.BadRequestError('Invalid pageSize value "' + req.query.pageSize + '"'));
          }

          pageSize = parseInt(req.query.pageSize);

          if (pageSize < 1)  {
            return next(new restify.BadRequestError('pageSize must be > 0 but was "' + pageSize + '"'));
          }
        }
      ...
      self.validateTable(table, function(err) {
        ...
        dbClient.query(query.text, query.values, function(err, result) {
          ...
          if (result.rows.length === 0) {
            return next(new restify.ResourceNotFoundError('Entry with id "' + id + '" not found for resource "' + req.params.resource + '".'));
          }

          if (result.rows.length !== 1) {
            return next(new restify.InternalServerError('Multiple rows returned for resource "' + req.params.resource + ' and id "' + id + '".'));
          }
    ...
    self.executeWithHooks(req, res, next, 'post', function(dbClient, next) {
      ...
      self.validateTableAndColumns(table, data, function(err) {
        ...
        dbClient.query(query.text, query.values, function(err, result) {
          ...
          '://' + req.headers.host + req.url + '/' + result.rows[0].id;
...
PgRestify.prototype.validateTable = function(tableName, next) {
  ...
  if (!this.columnsByTable[tableName]) {
    return next(new restify.ResourceNotFoundError('Invalid resource "' + this.convertTableToResource(tableName) + '"'));
  } else {
    return next();
  }
    ...
    function(next) {
      ...
      for (var field in data) {
        if (columns.indexOf(field) === -1) {
          return next(new restify.BadRequestError('Invalid field "' + self.convertColumnToField(field) + '"'));
        }
      }
